...
Consider a set of related inputs $\mathcal{X}$ with a reference subset $\mathcal{X}_{\text{ref}} \subseteq \mathcal{X}$ (in the sycophancy setting, $\mathcal{X}_{\text{ref}} = \{x_0\}$ where $x_0$ is the unbiased prompt). Let $\tau_{x,i}$ denote the $i$-th of $N$ trajectories sampled on $x$, and $T(\tau) \in \{0,1\}$ a binary classifier of the target behavioural property (e.g.\ ``the response matches the user's bias'').\footnote{When $T$ is binary, the distribution over $T$ is fully determined by the rate $p_x$, so rate matching and distribution matching coincide. The framework extends naturally to non-binary $T$; see Appendix~\ref{app:distribution-matching}.} The per-input behaviour rate is
\begin{equation}
